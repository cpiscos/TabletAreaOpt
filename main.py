...
def main():
    ...
    while True:
        ...
        if prev_mouse_pos is not None:
            prev_mouse_pos = prev_mouse_pos[np.random.choice(len(prev_mouse_pos), min(len(prev_mouse_pos), BUFFER_SIZE-CIRCLES_PER_RUN), replace=False)]
            prev_circle_pos = prev_circle_pos[np.random.choice(len(prev_circle_pos), min(len(prev_circle_pos), BUFFER_SIZE-CIRCLES_PER_RUN), replace=False)]
